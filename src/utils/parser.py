...
class CocoVidParser(COCO):
    """
    Taken and partially modified from https://github.com/open-mmlab/mmtracking/blob/master/mmtrack/datasets/parsers/coco_video_parser.py
    ...
    def __init__(self, annotation_file=None, load_img_as_vid=False):
        ...
        super(CocoVidParser, self).__init__(annotation_file=annotation_file)
        ...
        if "annotations" in self.dataset:
            for ann in self.dataset["annotations"]:
                ...
                if "instance_id" in ann:
                    ...
                    if (
                        "video_id" in ann
                        and ann["instance_id"] not in vidToInstances[ann["video_id"]]
                    ):
    ...
    def get_vid_id_from_img_id(self, imgId):
        """Get video id from given image id.

        Args:
            imgId (int): The given image id.

        Returns:
            int: Video id of given image id.
        """
        return self.imgs[imgId]["video_id"]
    ...
    def get_labels_from_vid_id(self, vidId):
        """Get a dict labels with their corresponding annotation ids from given video id.

        Args:
            vidId (int): The given video id.

        Returns:
            dict[int, list[int]]: Labels with their corresponding annotation ids.
        """
        img_ids = self.get_img_ids_from_vid(vidId)
        ann_ids = [self.imgToAnns[img_id] for img_id in img_ids]
        labels = {}
        for ann_id in ann_ids:
            for ann in ann_id:
                if ann["cell_id"] not in labels:
                    labels[ann["cell_id"]] = []
                labels[ann["cell_id"]].append(ann["id"])
        return labels
